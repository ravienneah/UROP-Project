...
data_waves124$early_life_heavy_drinking <- levels(data_waves124$H1TO17) <- c(6, 5, 4, 3, 2, 1, 0)
data_waves124 <- data_waves124 |> mutate(early_life_subst_use =
                                           early_life_alc_use + early_life_heavy_drinking)
```
...
```{r}
#perceived discriminiation
data_waves124$teacher_discrimination <- levels(data_waves124$H2ED19) = c(1, 2, 3, 4, 5)
data_waves124$peer_prejudice <- levels(data_waves124$H2ED17) = c(5, 4, 3, 2, 1)
data_waves124 <- data_waves124 |> mutate(perceived_discrimination = teacher_discrimination + peer_prejudice)

#perceived social support
data_waves124$adult_support <- levels(data_waves124$H2PR1) = c(1, 2, 3, 4, 5)
data_waves124$teacher_support <- levels(data_waves124$H2PR2) = c(1, 2, 3, 4, 5)
data_waves124$parent_support <- levels(data_waves124$H2PR3) = c(1, 2, 3, 4, 5)
data_waves124$peer_support <- levels(data_waves124$H2PR4) = c(1, 2, 3, 4, 5)
