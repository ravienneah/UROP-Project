...
#How often have you had problems with your family, friends, or people at work or school because of your drinking?
cx_met_lifetime <- cx_met_lifetime |>
  mutate(
    sud_persistence3 = as.numeric(
      H4TO49 %in% c("(1) (1) 1 time", "(2) (2) More than 1 time")
    )
  )
...
cx_met_lifetime <- cx_met_lifetime |>
  mutate(
    ...
    # Q55 coding (unchanged)
    sud_losscontrol2 = case_when(
      H4TO54 != "(1) (1) Yes" ~ as.numeric(H4TO55 == "(1) (1) Yes"),
      TRUE ~ 0
    ),
    
    # Fixed Q56 coding
    sud_losscontrol4 = case_when(
      H4TO56 == "(1) (1) Yes" ~ 1,
      H4TO56 == "(0) (0) No" ~ 0,
      TRUE ~ NA_real_
    )
...
cx_met_lifetime <- cx_met_lifetime |>
  mutate(
    ...
    sud_persistence = sud_persistence1 + sud_persistence2 + sud_persistence3
  )
...
cx_met_lifetime <- cx_met_lifetime %>%
  mutate(
    # Code Q55 responses (only when Q54 == 0)
    sud_losscontrol2 = case_when(
      H4TO54 == "(0) (0) No" ~ as.numeric(H4TO55 == "(1) (1) Yes"),
      H4TO54 == "(1) (1) Yes" ~ 0,  # Skipped Q55 → code as 0
      TRUE ~ NA_real_
    )
  )

# Step 2: NOW filter out NAs for non-skip questions
cx_met_lifetime <- cx_met_lifetime %>%
  filter(
    !is.na(H4TO46),   # Q46
    !is.na(H4TO47),   # Q47
...
# Step 3: Verify
table(cx_met_lifetime$H4TO54, useNA = "always")  # Should show BOTH 0 and 1
table(cx_met_lifetime$H4TO55, useNA = "always")  # Should show some non-NA if Q54=0 cases exist
```
```{r}
cx_met_lifetime <- cx_met_lifetime %>%
  mutate(
    # Convert Q54 to numeric (0=No, 1=Yes)
    sud_losscontrol3 = case_when(
      H4TO54 == "(0) (0) No" ~ 0,
      H4TO54 == "(1) (1) Yes" ~ 1,
      TRUE ~ NA_real_
    ...
    # Handle Q55 (NA when Q54=1 is legitimate skip → code as 0)
    sud_losscontrol2 = case_when(
      H4TO54 == "(0) (0) No" ~ as.numeric(H4TO55 == "(1) (1) Yes"), # Actual answers
      H4TO54 == "(1) (1) Yes" ~ 0, # Legitimate skips → 0
      TRUE ~ NA_real_ # Only for invalid cases (shouldn't exist per your counts)
    ),
    
    # Q56 (asked to everyone)
    sud_losscontrol4 = case_when(
      H4TO56 == "(1) (1) Yes" ~ 1,
      H4TO56 == "(0) (0) No" ~ 0,
      TRUE ~ NA_real_
  ...
  # Only filter out rows with NA in NON-SKIP variables
  filter(
    !is.na(H4TO46) &   # Filtered (no NA in your data)
    !is.na(H4TO47) &   # Filtered (no NA)
    !is.na(H4TO51) &   # Filtered (no NA)
    !is.na(H4TO52) &   # Filtered (no NA)
    !is.na(H4TO53)     # Filtered (no NA)
  )
```


```{r }
# Step 3: Proceed with composite creation
cx_met_lifetime <- cx_met_lifetime %>%
  mutate(
    sud_losscontrol = sud_losscontrol1 + sud_losscontrol2 + sud_losscontrol3 + sud_losscontrol4,
    drinking_exp = sud_progression + sud_preoccupation + sud_losscontrol + sud_persistence
...
# Export clean data
AUD_indicators <- cx_met_lifetime %>%
  select(AID, substance_misuse_manual, substance_misuse, drinking_exp,
         starts_with("sud"), H4TO47:H4TO62)

#write_csv(AUD_indicators, "problematic_alcohol_misuse_addhealth.csv")
```
